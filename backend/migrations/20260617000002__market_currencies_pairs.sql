...
CREATE TABLE market.trading_pairs (
    ...
    base_currency_id    BIGINT          NOT NULL REFERENCES market.currencies (id),
    quote_currency_id   BIGINT          NOT NULL REFERENCES market.currencies (id),
...
CREATE INDEX idx_trading_pairs_base_currency ON market.trading_pairs (base_currency_id);
CREATE INDEX idx_trading_pairs_quote_currency ON market.trading_pairs (quote_currency_id);

CREATE TABLE market.user_favorite_pairs (
    user_id             BIGINT          NOT NULL REFERENCES iam.users (id) ON DELETE CASCADE,
    trading_pair_id     BIGINT          NOT NULL REFERENCES market.trading_pairs (id) ON DELETE CASCADE,
    added_at            TIMESTAMPTZ     NOT NULL DEFAULT NOW(),

    PRIMARY KEY (user_id, trading_pair_id)
);

CREATE INDEX idx_user_favorite_pairs_trading_pair ON market.user_favorite_pairs (trading_pair_id);
